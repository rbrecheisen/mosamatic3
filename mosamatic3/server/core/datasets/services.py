...
def create_empty_output_dataset_for_user_id(
    *,
    name: str,
    user_id,
...
def append_output_files_to_dataset(dataset: Dataset, files: list[OutputDatasetFile]) -> None:
    root = dataset_upload_root(dataset.owner_id, dataset.id)
    ...
    for out in files:
        ...
        if existing is None:
            DatasetFile.objects.create(
                dataset=dataset,
                relative_path=relative_path,
                size_bytes=len(out.content),
            )
        else:
            existing.size_bytes = len(out.content)
            existing.save(update_fields=['size_bytes'])
...
def create_output_dataset_for_user_id(name: str, files: list[OutputDatasetFile], user_id, source_task_key=None, source_task_id=None) -> Dataset:
    ...
    append_output_files_to_dataset(dataset, files)
    return dataset
