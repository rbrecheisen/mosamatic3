...
@app.post("/api/datasets", response_model=UploadResult, status_code=status.HTTP_201_CREATED)
async def create_dataset(
    ...
    return UploadResult(dataset=DatasetRead.model_validate(dataset, from_attributes=True))
...
def list_datasets(
    ...
    for dataset in datasets:
        files = session.exec(select(DatasetFile).where(DatasetFile.dataset_id == dataset.id)).all()
        result.append(
            DatasetSummary(
                id=dataset.id,
                name=dataset.name,
...
@app.post("/api/uploads", response_model=UploadResult, status_code=status.HTTP_201_CREATED)
